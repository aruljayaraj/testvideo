...
const QQStepInd: React.FC = () => {
  ...
  let { id, rfqType, step } = useParams<any>();

  return (<>
      <div className="mb-4">
        <ol className="step-indicator">
            <li className={ (!step || (step && step === '1'))? 'active': (step && step > '1')? 'complete' : '' }>
              <IonRouterLink {... (id && step && qq && qq.p_title && qq.p_desc) ? {href: `${basename}/layout/add-localquote/${rfqType}/${id}/${authUser.ID}/1`} : {}}>
            ...
            <li className={ (step && step === '2')? 'active': (step && step > '2')? 'complete' : '' }>
              <IonRouterLink {... (id && step && qq && qq.buscats && qq.buscats.length > 0) ? {href: `${basename}/layout/add-localquote/${rfqType}/${id}/${authUser.ID}/2`} : {}}>
            ...
            <li className={ (step && step === '3')? 'active': (step && step > '3')? 'complete' : '' }>
              <IonRouterLink {... (id && step && qq && qq.attachments && (qq.attachments.document.length > 0 || qq.attachments.audio.length > 0 || qq.attachments.video.length > 0)) ? {href: `${basename}/layout/add-localquote/${rfqType}/${id}/${authUser.ID}/3`} : {}}>
            ...
            <li className={ (step && step === '4')? 'active': (step && step > '4')? 'complete' : '' }>
              <IonRouterLink {... (id && step && qq && qq.quotation_req_date) ? {href: `${basename}/layout/add-localquote/${rfqType}/${id}/${authUser.ID}/4`} : {}}>
