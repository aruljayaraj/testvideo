...
const PreNews: React.FC = () => { 
  const preResults = useSelector( (state:any) => state.search.preResults.news); 
  const { basename } = lfConfig;
  return (<>
     { preResults && preResults && preResults.length > 0 && <IonCard className="card-center my-4">
    <IonCardHeader color="titlebg">
        <IonCardTitle className="fs-18">Business News</IonCardTitle>
    </IonCardHeader>
    ...
    <IonCardContent>
        ...
        { preResults.map((item: any) => { 
            return (
            <IonItem lines="none" key={nanoid()} routerLink={`${basename}/press-release/${item.mem_id}/${item.id}`}>
                {(isPlatform('desktop')) && <IonAvatar slot="start" color="greenbg">
                    <i className="fa fa-newspaper-o fa-lg green" aria-hidden="true"></i>
                </IonAvatar> }
                <IonLabel>
